<script lang="ts">
    ...
    function organizarCarreras() {
        const ordenadas = [...solicitudes].sort((a, b) => 
            new Date(a.hora_servicio).getTime() - new Date(b.hora_servicio).getTime()
        );
    ...
    function enviarTodosAWhatsApp() {
        carrerasOrganizadas.forEach(grupo => enviarAWhatsApp(grupo));
    }
...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2">
            <button class="btn btn-outline-primary" on:click={cargarSolicitudes} aria-label="Actualizar solicitudes">
                <i class="bi bi-arrow-clockwise me-1"></i> Actualizar
            </button>
            <button class="btn btn-success" on:click={enviarTodosAWhatsApp}>
                <i class="bi bi-whatsapp me-1"></i> Enviar todos
            </button>
        </div>
    </div>
            ...
            {#each carrerasOrganizadas as grupo, i}
                <div class="accordion-item mb-3 border-0 shadow-sm">
                    <h2 class="accordion-header" id={`heading${i}`}>
                        <button
                            class="accordion-button d-flex align-items-center justify-content-between {i !== 0 ? 'collapsed' : ''}"
                            type="button"
                                        ...
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div class="flex-grow-1">
                                                <div class="d-flex align-items-center mb-2">
                                                    ...
                                                    <span class="badge bg-secondary rounded-pill">
                                                        {solicitud.pasajeros} {solicitud.pasajeros === 1 ? 'pasajero' : 'pasajeros'}
                                                    </span>
                                                ...
                                                <div class="d-flex align-items-center text-muted small">
                                                    ...
                                                    {new Date(solicitud.hora_servicio).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                                                    {#if solicitud.notas}
                                                        <span class="ms-3">
                                            ...
                                            <div class="btn-group ms-3">
                                                <button 
                                                    ...
                                                    on:click={() => enviarAWhatsApp(grupo)}
                                                >
                                                    <i class="bi bi-whatsapp me-1"></i>
                                                    <span class="d-none d-md-inline">Enviar grupo</span>
                                                </button>
                                            </div>
                                        </div>
                                    </div>
